...
// Compiled only if LOADBALANCE = 0
  ...
  /*
   * We have NB_THREADS amount of threads, split the picture
   * in rows corresponding to the amount of threads.
   * I. Split the picture height with NB_THREADS and let each
   *    thread compute the region:
   *      upper bound: (height/NB_THREADS) * (thread_id + 1)
   *      lower bound: (height/NB_THREADS) * thread_id
   */
  // Define the region compute_chunk() has to compute
  // restrict the width depending on thread_id
  int height_tick = (parameters->height/NB_THREADS);
  parameters->begin_h = height_tick * args->id;
  int end_h = 0;
  if (args->id == NB_THREADS - 1) {
    end_h = parameters->height;
  }
  else {
    end_h = height_tick * (args->id + 1);
  }
  parameters->end_h = end_h;
...
#if LOADBALANCE == 1
  /*
   * Split depending on the thread_id
   * Split by rows, C works faster with rows
   * Nearby rows should have about the same computational complexitiy
   */
  int i = args->id;
  for (; i < parameters->height; i+= NB_THREADS) {
